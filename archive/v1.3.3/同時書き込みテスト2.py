...
def write_data(key, value):
    with dict_sqlite.DictSQLite('./temp.db', conflict_resolver=False, lock_file="test.lock") as db:
        db[key] = value
...
time.sleep(1)
db = dict_sqlite.DictSQLite('./temp.db', conflict_resolver=False, lock_file="test.lock")  # メモリ内データベースを使用
# 書き込まれたデータを確認
for i in range(100):
