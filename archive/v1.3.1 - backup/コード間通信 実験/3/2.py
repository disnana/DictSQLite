...
class DictSQLite:
    def __init__(self, db_name, table_name='main', schema=None, multi_connect="auto", lock_file="./db.lock"):
        ...
        self.manager = ManagerWithList(address=('127.0.0.1', 50000), authkey=b'secret')
        self.manager.register('get_shared_list')
        self.manager.connect()
        self.shared_list = self.manager.get_shared_list()
        print(self.shared_list)
        print(f'Shared list contents: {self.shared_list}')

    def _process_queue(self):
        while True:
            operation, args, kwargs, result_queue = self.operation_queue.get()
            try:
                result = self.shared_list.put(*args, **kwargs)
