...
def test_dict_sqlite():
    # DictSQLite_beta クラスのインスタンスを作成
    db = dict_sqlite.DictSQLite_beta("sample.db", journal_mode="WAL")
    # データ追加
    db['name'] = 'Alice'
    db['age'] = '30'
    print("データ追加後:", db)
    ...
    # テーブルの切り替え
    # テーブルの主導作成（手動で作成しなくてもswitch_tableで自動で作成される）
    ...
    db.close()
    # コンテキストマネージャの使用
    with dict_sqlite.DictSQLite_beta("sample.db") as context_db:
