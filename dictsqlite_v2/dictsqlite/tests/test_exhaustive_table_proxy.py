#!/usr/bin/env python3
"""
網羅的テストスイート - TableProxy 全メソッドの詳細検証

このテストファイルは、TableProxyクラスの全メソッドを網羅的にテストし、
戻り値の型・値を厳密に検証します。
...
- Prefix mode vs Separate mode の動作差異
"""
...
# Rust拡張モジュールが利用可能か確認
try:
    from dictsqlite import DictSQLiteV4, is_native_available
...
@pytest.mark.skipif(not DICTSQLITE_V4_AVAILABLE, reason="DictSQLiteV4 module not built")
class TestTableProxyItemAccess:
    """TableProxyのアイテムアクセス検証"""
...
@pytest.mark.skipif(not DICTSQLITE_V4_AVAILABLE, reason="DictSQLiteV4 module not built")
class TestTableProxyContainsLen:
    """TableProxyの__contains__と__len__検証"""
...
@pytest.mark.skipif(not DICTSQLITE_V4_AVAILABLE, reason="DictSQLiteV4 module not built")
class TestTableProxyAccessMethods:
    """TableProxyのアクセスメソッド検証"""
...
@pytest.mark.skipif(not DICTSQLITE_V4_AVAILABLE, reason="DictSQLiteV4 module not built")
class TestTableProxyGetPopSetdefault:
    """TableProxyのget/pop/setdefault検証"""
...
@pytest.mark.skipif(not DICTSQLITE_V4_AVAILABLE, reason="DictSQLiteV4 module not built")
class TestTableProxyUpdateClear:
    """TableProxyのupdate/clear検証"""
...
class TestTableProxyReprStrEq:
    ...
    def test_repr_returns_string(self):
        ...
        with windows_safe_temp_db() as db_path:
            ...
            # テーブル名が含まれているか確認
            assert "users" in result or "v1" in result or "k1" in result
    ...
    def test_eq_between_table_proxies(self):
        """テーブルプロキシ同士の比較"""
        with windows_safe_temp_db() as db_path:
            db = DictSQLiteV4(db_path, storage_mode="bytes")
            
            users1 = db.table("users")
            users2 = db.table("users")  # 同じテーブル
...
# =============================================================================
# セクション9: Prefix mode vs Separate mode
# =============================================================================

@pytest.mark.skipif(not DICTSQLITE_V4_AVAILABLE, reason="DictSQLiteV4 module not built")
class TestTableModeDifferences:
    """テーブルモードの動作差異検証"""
    ...
    def test_prefix_mode_isolation(self):
        ...
        with windows_safe_temp_db() as db_path:
            ...
            # メインDBからは見えない（または別のプレフィックス付きキー）
            assert "same_key" not in db or db.get("same_key") != b"user_value"
    ...
    def test_prefix_mode_keys_isolated(self):
        """Prefixモードでkeys()がテーブル単位"""
        with windows_safe_temp_db() as db_path:
            db = DictSQLiteV4(db_path, table_mode="prefix", storage_mode="bytes")
            ...
            assert user_keys == {"u1", "u2"}
            assert product_keys == {"p1"}
            
            db.close()

    def test_separate_mode_keys_isolated(self):
        """Separateモードでkeys()がテーブル単位"""
        with windows_safe_temp_db() as db_path:
            db = DictSQLiteV4(db_path, table_mode="separate", storage_mode="bytes")
...
@pytest.mark.skipif(not DICTSQLITE_V4_AVAILABLE, reason="DictSQLiteV4 module not built")
class TestTableProxyStorageModes:
    """TableProxyのストレージモード別テスト"""
...
@pytest.mark.skipif(not DICTSQLITE_V4_AVAILABLE, reason="DictSQLiteV4 module not built")
class TestTableProxyPersistence:
    """TableProxyの永続化テスト"""
...
class TestTableProxyEdgeCases:
    ...
    def test_special_table_names(self):
        ...
        with windows_safe_temp_db() as db_path:
            ...
            # 数字で始まるテーブル名
            table1 = db.table("123table")
            table1["key"] = b"value"
            assert table1["key"] == b"value"
            
            # スペースを含むテーブル名
            table2 = db.table("my table")
            table2["key"] = b"value"
            assert table2["key"] == b"value"
    ...
    def test_large_data_in_table(self):
        ...
        with windows_safe_temp_db() as db_path:
            ...
            assert users["user_500"] == b"data_500"
            assert users["user_999"] == b"data_999"
