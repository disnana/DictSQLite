...
- **パッケージバージョン**: v2.1.3（PyPIでの公開バージョン）
...
- ストレージへの flush 失敗時に write buffer を復元し、未保存データの消失を防止
- delete/clear 後に未 flush の書き込みが値を復活させる問題を修正
- batch 取得のキャッシュミス時に一括 SQLite 読み込みを使用
- warm cache のメモリ使用量管理を改善
- separate table の永続化と圧縮/展開の整合性を修正
- ベンチマークと GitHub Actions の結果 Summary を拡充
