...
impl AsyncDictSQLite {
    ...
    fn new(db_path: String, capacity: usize, persist_mode: &str, buffer_size: usize) -> PyResult<Self> {
        use std::str::FromStr;
    ...
    fn batch_get(&self, keys: Vec<String>, py: Python) -> PyResult<Vec<Option<PyObject>>> {
        ...
        let (cached_results, cache_misses): (Vec<_>, Vec<_>) = py.allow_threads(|| {
            ...
            let results: Vec<(usize, Option<Vec<u8>>)> = keys.par_iter().enumerate().map(|(idx, key)| {
                (idx, cache.get(key).map(|value| value.clone()))
            }).collect();
        ...
        Ok(final_results.into_iter().map(|(_, opt_value)| {
