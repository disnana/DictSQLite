...
class PerformanceProfiler:
    ...
    def benchmark_sync_operations(self, db_path=None):
        ...
        for size in test_sizes:
            ...
            print(
                f"    読み取り: {memory_stats['execution_time']:.4f}s, メモリ増加: {memory_stats['memory_diff']:.2f}MB")
            ...
            print(
                f"    バルク挿入: {memory_stats['execution_time']:.4f}s, メモリ増加: {memory_stats['memory_diff']:.2f}MB")
            ...
            print(
                f"    バルク読み取り: {memory_stats['execution_time']:.4f}s, メモリ増加: {memory_stats['memory_diff']:.2f}MB")
    ...
    async def benchmark_async_operations(self, db_path=None):
        ...
        for size in test_sizes:
            ...
            print(
                f"    非同期挿入: {memory_stats['execution_time']:.4f}s, メモリ増加: {memory_stats['memory_diff']:.2f}MB")
            ...
            print(
                f"    非同期読み取り: {memory_stats['execution_time']:.4f}s, メモリ増加: {memory_stats['memory_diff']:.2f}MB")
            ...
            print(
                f"    非同期バルク: {memory_stats['execution_time']:.4f}s, メモリ増加: {memory_stats['memory_diff']:.2f}MB")
    ...
    def benchmark_concurrency(self, db_path=None):
        ...
        for thread_count in thread_counts:
            ...
            print(
                f"    並行実行({thread_count}スレッド): {memory_stats['execution_time']:.4f}s, メモリ増加: {memory_stats['memory_diff']:.2f}MB")
    ...
    def generate_report(self, sync_results, async_results, concurrent_results):
        ...
        for size, operations in sync_results.items():
            ...
            for op_name, stats in operations.items():
                ...
                all_ops_results.append(
                    (f"{op_name}({size})", ops_per_sec, stats['execution_time'], stats['memory_diff']))
                print(
                    f"    {op_name:15}: {ops_per_sec:8,.0f} ops/sec ({stats['execution_time']:6.4f}s) メモリ: {stats['memory_diff']:+6.2f}MB")
        ...
        for thread_count, stats in concurrent_results.items():
            ...
            concurrent_ops_results.append(
                (f"{thread_count}スレッド", throughput, stats['execution_time'], stats['memory_diff']))
            print(
                f"  {thread_count:2d}スレッド: {throughput:8,.0f} ops/sec ({stats['execution_time']:6.4f}s) メモリ: {stats['memory_diff']:+6.2f}MB")
