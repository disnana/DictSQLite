...
@pytest.fixture()
def db(db_path):
    """FastDictSQLite のインスタンスを提供し、テスト後にクローズ。"""
...
def test_evil_payload_is_not_executed_and_value_is_str(db: FastDictSQLite):
...
def test_mixed_payload_is_blocked_and_string_returned(db: FastDictSQLite):
...
def test_policy_allows_project_function(db_path):
    ...
    d = FastDictSQLite(str(db_path), **kwargs)
    try:
