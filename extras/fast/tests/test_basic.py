...
def test_basic_crud_and_repr(db: FastDictSQLite):
    db["a"] = 1
    db["b"] = "text"
    db["c"] = 3.14
    db["d"] = True
...
def test_top_level_synced_list_and_set(db: FastDictSQLite):
    db["lst"] = [1, 2]
    db["st"] = {1, 2}
...
def test_recursive_dict_and_nested_behavior(db: FastDictSQLite, initial):
    db["cfg"] = initial
...
def test_contains_keys_and_clear_table(db: FastDictSQLite):
    db["k1"] = "v1"
    db["k2"] = "v2"
